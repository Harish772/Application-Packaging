...
Try {
	...
	If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
       ...
       $checkforVC15x86 = Get-InstalledApplication -name "Microsoft visual c++ 2015-2019 Redistributable (x86)"
		...
		Execute-MSI -Action Install -Path 'Esprit.msi' -Transform 'ESprit_2021_USA_E.Mst' 
       ...
       # Execute-Process -Path "$dirFiles\PostInstall\ECOP_Release_(ws-PROD)-(FULL)_(Esprit2020-without-supporting-E2020-libraries)_06.04.2020_10.02.05.exe" -Parameters "/VERYSILENT"
        ...
        Write-log -Message "Start...Importing the Registry 2  to UserHive." -LogType 'CMTrace'
        ...
        Write-log -Message "End...Importing the Registry 2  to UserHive." -LogType 'CMTrace'
        ...
        If (!(Test-Path $DestFolder1)) {
            ...
            Start-sleep -s 10
             ...
             #-------------------Fix for KnowledgeBase Connection--------------------------------------------------------------------------------------------------

            # Create a Primary subkey Key under Main Key 
              Write-log -Message "Start...Creating Registry Entries for KnowledgeBase Connection on the system." -LogType 'CMTrace'

            New-Item -Path HKLM:\SOFTWARE\Wow6432Node\D.P.Technology\ESPRIT\Setup -Force

            # Create the Key properties 
            New-ItemProperty -Path HKLM:\SOFTWARE\Wow6432Node\D.P.Technology\ESPRIT\Setup -Name DBDatabase -PropertyType String -Value KBM -Force
            New-ItemProperty -Path HKLM:\SOFTWARE\Wow6432Node\D.P.Technology\ESPRIT\Setup -Name DBUsername -PropertyType String -Value kbm -Force
            New-ItemProperty -Path HKLM:\SOFTWARE\Wow6432Node\D.P.Technology\ESPRIT\Setup -Name DBPassword -PropertyType String -Value kbm -Force
            New-ItemProperty -Path HKLM:\SOFTWARE\Wow6432Node\D.P.Technology\ESPRIT\Setup -Name DBServer -PropertyType String -Value IS1SQL38\IS1SQL38 -Force

             Write-log -Message "End...Creating Registry Entries for KnowledgeBase Connection on the system." -LogType 'CMTrace'
             #######################################################################################################################################################################################################################################
