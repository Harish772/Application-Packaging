...
Try {
	...
	[string]$appVendor = 'Barco N.V'
	[string]$appName = 'ClickShare'
	[string]$appVersion = '4.1.1-b2'
	[string]$appArch = 'NA'
	[string]$appLang = 'EN'
	[string]$appRevision = '01'
	[string]$appScriptVersion = '1.0.0'
	[string]$appScriptDate = '9/2/2020'
	...
	If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
        ...
        Execute-MSI -Action 'Install' -Path "ClickShare_Installer.msi" -Transform "ClickShare.Mst" 
		...
		## <Perform Post-Installation tasks here>
         ...
         New-Item -Path "HKLM:\SOFTWARE" -Name "ClickShare" -Force -ErrorAction SilentlyContinue
         New-ItemProperty -Path "HKLM:\SOFTWARE\ClickShare" -Name "Installed" -PropertyType "String" -Value "1" -Force -ErrorAction SilentlyContinue
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		# <Perform Uninstallation tasks here>
         ...
         Show-InstallationProgress
         #Execute-Process -Path "$env:USERPROFILE\AppData\Local\ClickShare\update.exe" -Parameters " --uninstall" -wait
         
         &cmd /c "$env:USERPROFILE\AppData\Local\ClickShare\update.exe" --uninstall

         ##############################################################################

         $users = Get-ChildItem C:\users
         foreach ($user in $users){
         $folder = "$($user.fullname)\AppData\Local\ClickShare"
         $file = "$($user.fullname)\AppData\Local\ClickShare\Update.exe"
         $folderBarco = "$($user.fullname)\AppData\Local\Barco"
         $folderBarClick = "$($user.fullname)\AppData\Local\Barco\ClickShare Client"
         ...
         If(Test-Path $folderBarClick){
         Remove-Item -Path $folderBarClick -Recurse -Force }
         

         If(Test-Path $folderBarco){
         Remove-Item -Path $folderBarco -Recurse -Force }
         
         }

         Set-ItemProperty -Path "HKLM:\SOFTWARE\ClickShare" -Name "Installed" -Value "0"
