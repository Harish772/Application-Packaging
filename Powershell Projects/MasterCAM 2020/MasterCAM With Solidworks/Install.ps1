...
$OldAppDetection1 = "C:\Program Files\McamForSW2019\McamAdvConfig.exe"
$OldAppDetection2 = "C:\Program Files\McamForSW2019\Extensions\ToolManager.exe"
$OldAppDetection3 = "C:\Program Files\McamForSW2019\common\Editors\CIMCOEdit8\CimcoDNC.exe"
...
if((Test-Path $OldAppDetection1) -or (Test-Path $OldAppDetection2) -or (Test-Path $OldAppDetection3)) {
     ...
     Write-Host ("Older Version of the Application (MasterCAM for SolidWorks 2019 detected). Executing the Uninstall Logic")
     Write-Log ("Older Version of the Application (MasterCAM for SolidWorks 2019 detected). Executing the Uninstall Logic")
...
if ($processesToKill.Count -gt 0) {
    ...
    foreach ($proc in ($allprocesses | Where-Object {$processesToKill -contains $_.Name})) {
	 ...
	 ##* UNINSTALLATION
	 ##*===============================================
$OldappScriptFilename = "MasterCAMforSW2019"
Write-Log "Older Version of the Application($oldappScriptFilename) detected. Executing the Uninstall Logic"
    $procstartinfo = new-object System.Diagnostics.ProcessStartInfo
    $procstartinfo.FileName = "$scriptpath\Source\7.Uninstall 2019\Mastercam_SW_Uninstaller.EXE"
...
if((Test-Path $AppDetection1) -or (Test-Path $AppDetection2) -or (Test-Path $AppDetection3)) {
     ...
     ##*===============================================
	 ##* PRE-INSTALLATION (1. MasterCAM 2020)
	 ##*===============================================
     


Write-Log "PRE-INSTALLATION Phase : Installing CNC_MasterCam_2020_22.0.25841.0"

Write-Host "PRE-INSTALLATION Phase: Installing CNC_MasterCam_2020_22.0.25841.0"    


    $MCappScriptFilename = "CNC_MasterCam_2020_22.0.25841.0"
    $varx = "$scriptPath\Source\1.CNC_MasterCam_2020_22.0.25841.0\CNC_MasterCam_2020_22.0.25841.0-Install.ps1"
    $proc1startinfo = new-object System.Diagnostics.ProcessStartInfo
    $proc1startinfo.FileName = "C:\Windows\System32\WindowsPowerShell\v1.0\powershell.exe"
    $proc1startinfo.Arguments = "-ExecutionPolicy Bypass -File `"$Varx`" -Force"
...
if (($proc1.ExitCode -eq 0) -or ($proc1.ExitCode -eq 3010) -or ($proc1.ExitCode -eq 1641) -or ($proc1.ExitCode -eq 1603)) {
    ...
    Write-Log ("$MCappScriptFilename Install completed successfully with code: " + $proc1.ExitCode)
}
else {

    Write-Log ("$MCappScriptFilename Install failed with code " + $proc1.ExitCode) -type Error
    exit $exitcode1
}
    Write-Log ("Start - Deferring clean up of packages/files, if any exist after $MCappScriptFilename Install")
    Write-Host ("Start - Deferring clean up of packages/files, if any exist after $MCappScriptFilename Install")
    Start-sleep -s 180
    Write-Log ("End - Deferring clean up of packages/files, if any exist after $MCappScriptFilename Install")
    Write-Host ("End - Deferring clean up of packages/files, if any exist after $MCappScriptFilename Install")
     ...
     ##*===============================================
	 ##* INSTALLATION (2. MasterCAM for Solidworks 2020(Main))
	 ##*===============================================
...
Write-Host "Installing $appScriptFilename.exe (Main)"
    ...
    $proc2startinfo.FileName = "$scriptpath\Source\2.MastercamForSOLIDWORKS\setup.exe"
...
if (($proc2.ExitCode -eq 0) -or ($proc2.ExitCode -eq 3010) -or ($proc2.ExitCode -eq 1641) -or ($proc2.ExitCode -eq 1603)) {
    ...
    Write-Log ("Start - Deferring clean up of packages/files, if any exist after 1.MasterCAM for SW 2020 (Main) Install")
    Write-Host ("Start - Deferring clean up of packages/files, if any exist after 1.MasterCAM for SW 2020 (Main) Install")
    Start-sleep -s 180
    Write-Log ("End - Deferring clean up of packages/files, if any exist after 1.MasterCAM for SW 2020 (Main) Install")
    Write-Host ("End - Deferring clean up of packages/files, if any exist after 1.MasterCAM for SW 2020 (Main) Install")

     
     
     ##*===============================================
	 ##* INSTALLATION (3.MasterCAM for SW Update 1)
	 ##*===============================================
     
$AppInstall3 = "3.MasterCAM for SW Update 1"
...
Write-Host "Installing $AppInstall3"
    ...
    $proc3startinfo.FileName = "$scriptpath\Source\3.Update 1\mcfsw-Update1-PC8.exe"
...
if (($proc3.ExitCode -eq 0) -or ($proc3.ExitCode -eq 3010) -or ($proc3.ExitCode -eq 1641) -or ($proc3.ExitCode -eq 1603)) {
    ...
    Write-Host ("End - Deferring clean up of packages/files, if any exist after $AppInstall3 Install")
     ...
     ##*===============================================
	 ##* INSTALLATION (4.MasterCAM for SW Update 1.1)
	 ##*===============================================
     
$AppInstall4 = "4.MasterCAM for SW Update 1.1"
...
Write-Host "Installing $AppInstall4"
    ...
    $proc4startinfo.FileName = "$scriptpath\Source\4.Update 1.1\mastercam2020fsw-update1_1-patch.exe"
...
if (($proc4.ExitCode -eq 0) -or ($proc4.ExitCode -eq 3010) -or ($proc4.ExitCode -eq 1641) -or ($proc4.ExitCode -eq 1603)) {
    ...
    Write-Log ("$AppInstall4 main install completed successfully with code: " + $proc4.ExitCode)
}
else {

    Write-Log ("$AppInstall4 install failed with code " + $proc4.ExitCode) -type Error
    exit $exitcode4
}
    ...
    Write-Host ("End - Deferring clean up of packages/files, if any exist after $AppInstall4 Install")
     ...
     ##*===============================================
	 ##* INSTALLATION (5.MasterCAM for SW Update 2)
	 ##*===============================================
     
$AppInstall5 = "5.MasterCAM for SW Update 2"
...
Write-Host "Installing $AppInstall5"
    ...
    $proc5startinfo.FileName = "$scriptpath\Source\5.Update 2\mastercam2020fsw-update2-patch.exe"
...
if (($proc5.ExitCode -eq 0) -or ($proc5.ExitCode -eq 3010) -or ($proc5.ExitCode -eq 1641) -or ($proc5.ExitCode -eq 1603)) {
    ...
    Write-Log ("$AppInstall5 install completed successfully with code: " + $proc5.ExitCode)
}
else {

    Write-Log ("$AppInstall5 install failed with code " + $proc5.ExitCode) -type Error
    exit $exitcode5
}
    ...
    Write-Host ("End - Deferring clean up of packages/files, if any exist after $AppInstall5 Install")
     ...
     ##*===============================================
	 ##* INSTALLATION (6.HASP Sentinel Drivers )
	 ##*===============================================
...
$AppInstall6 = "6.HASP Sentinel Drivers"
Write-Log "Installing $AppInstall6"

if((Test-Path $HASPAppDetection1) -or (Test-Path $HASPAppDetection2)) {

    Write-Log ("$AppInstall6 already detected on the System..Terminating the Script")
    Write-Host ("$AppInstall6 already detected on the System..Terminating the Script")
}
    ...
    Write-Host "Installing $AppInstall6"
...
try {$proc6 = [System.Diagnostics.Process]::Start($proc6startinfo)}
    ...
    Write-Log "Error executing $scriptpath\$AppInstall6, file likely missing"
    exit 1603
}
$proc6.WaitForExit()
...
if (($proc6.ExitCode -eq 0) -or ($proc6.ExitCode -eq 3010) -or ($proc6.ExitCode -eq 1641) -or ($proc6.ExitCode -eq 1603)) {
    ...
    Write-Log ("$AppInstall6 install completed successfully with code: " + $proc6.ExitCode)
}
else {

    Write-Log ("$AppInstal6 install failed with code " + $proc6.ExitCode) -type Error
    exit $exitcode6
}

    Write-Log ("Start - Deferring clean up of packages/files, if any exist after $AppInstall6 Install")
    Write-Host ("Start - Deferring clean up of packages/files, if any exist after $AppInstall6 Install")
    Start-sleep -s 60
    Write-Log ("End - Deferring clean up of packages/files, if any exist after $AppInstall6 Install")
    Write-Host ("End - Deferring clean up of packages/files, if any exist after $AppInstall6 Install")
